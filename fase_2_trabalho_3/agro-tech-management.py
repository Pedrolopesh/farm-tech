...
try:
    conn = oracledb.connect(user='RM568480', password="030800", dsn='oracle.fiap.com.br:1521/ORCL')
        ...
        try:
            sql = """
                INSERT INTO TRANSACOES (
                    TIPO, DATA, CULTURA, CATEGORIA, DESCRICAO,
                    QUANTIDADE, UNIDADE, PRECO_UNITARIO, TOTAL, VALOR_ASSINADO,
                    METODO_PAGAMENTO, CONTRAPARTE, OBSERVACOES, CREATED_AT
                )
                VALUES (
                    :tipo, TO_DATE(:data, 'YYYY-MM-DD'), :cultura, :categoria, :descricao,
                    :quantidade, :unidade, :preco_unitario, :total, :valor_assinado,
                    :metodo_pagamento, :contraparte, :observacoes, TO_DATE(:created_at, 'YYYY-MM-DD')
                )
            """

            # remove campos não usados no SQL
            dados = {k: v for k, v in reg.items() if k != "id"}

            inst_cadastro.execute(sql, dados)
            conn.commit()
            print("🗄️  Registro gravado no banco Oracle com sucesso!")
        except Exception as e:
            print("⚠️  Falha ao gravar no banco:", e)
    else:
        print("⚠️  Sem conexão ativa com o Oracle. Registro só salvo em memória.")
...
def remover():
    ...
    if read_yesno("Confirmar remoção?", False):
        ...
        if conexao:
            try:
                sql_delete = "DELETE FROM TRANSACOES WHERE ID = :id"
                inst_exclusao.execute(sql_delete, {"id": _id})
                conn.commit()
                print("🗄️  Registro removido do Oracle com sucesso!")
            except Exception as e:
                print("⚠️  Falha ao remover no banco:", e)
        else:
            print("⚠️  Sem conexão ativa com o Oracle. Remoção só em memória.")
    else:
        ...
        try:
            sql_update = """
                UPDATE TRANSACOES
                 ...
                 WHERE ID = :id
            """
            inst_alteracao.execute(sql_update, reg)
            conn.commit()
            print("🗄️  Registro atualizado no Oracle com sucesso!")
        except Exception as e:
            print("⚠️  Falha ao atualizar no banco:", e)
    else:
        print("⚠️  Sem conexão ativa com o Oracle. Atualização só em memória.")
...
def listar_banco():
    if not conexao:
        print("Sem conexão com o Oracle.")
        return

    query = "SELECT ID, TIPO, CULTURA, TOTAL, METODO_PAGTO, CREATED_AT FROM TRANSACOES ORDER BY ID DESC"
...
def menu():
    while True:
        ...
        print("[9] Listar transações do Oracle")
        print("[0] Sair")
